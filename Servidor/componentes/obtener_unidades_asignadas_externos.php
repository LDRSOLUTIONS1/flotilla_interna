...
$sqlobtenerunidadasignada = "SELECT unid.img_unidad,
                            ...
                            WHERE unidasigcolab.id_usuario_externo";
...
if($resultado->num_rows > 0){
    ...
    function ordenarPorFechaexterno($a, $b) {
        return strtotime($a['fecha_asignacion']) - strtotime($b['fecha_asignacion']);
    }

    usort($arreglo_unidades, 'ordenarPorFechaexterno');
}
    ...
    if ($fila['id_estado_unidad'] == 3 && $fila['id_tipo_unidad'] == 1) {
        echo '<div class="card">
            ...
            <div class="card-body mt-3">
                ...
                <button type="button" id="btnentregaunidad" data-id="' . $fila['id_unidad'] . '" data-idusuarioexterno="' . $fila['id_usuario_externo'] . '" class="btn btn-success btn-sm  btnentregaunidad">Asignación presencial</button>
     </div>
        </div>';
    }
}
