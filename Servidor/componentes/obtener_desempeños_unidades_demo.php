...
$sqlobtenerunidadsubircomodato = "SELECT unid.img_unidad,
                ...
                md.nombre_1 AS nombre1masterdriver,
                md.nombre_2 AS nombre2masterdriver,
                md.apellido_paterno AS apellidopaternomasterdriver,
                md.apellido_materno AS apellidomaternomasterdriver,
                usr.avatar AS avatar_colaborador,
                usr_aut.avatar AS avatar_autorizador,
                usr_md.avatar AS avatar_masterdriver,
                epd.estado_prueba
            FROM asignacion_unidad_demo AS uda
                ...
                INNER JOIN colaboradores AS md
                ON uda.id_asignar_prueba_demo_master_driver = md.id_colaborador
                ...
                LEFT JOIN usuarios AS usr_md
                ON usr_md.id_colaborador = md.id_colaborador
    ...
    <table class="table table-hover tablaunidades" id="tablaUnidades">
        <thead class="table-light">
            <tr>
                ...
                <th class="titulostablaverificarcomodatodemo"></th> 
                <th class="titulostablaverificarcomodatodemo">Master Driver</th>
                <th class="titulostablaverificarcomodatodemo"></th> 
...
while ($fila = $resultado->fetch_assoc()) {
    if (($fila['id_persona_fisica'] || $fila['id_persona_moral']) && $fila['autorizacion'] === 'APROVADO') {
        ...
        echo '</td>
                    ...
                    <td class="text-center">
                    <img src="' . (empty($fila["avatar_masterdriver"]) ? "../../Cliente/img/default_avatar.png" : "https://ldrhsys.ldrhumanresources.com/Cliente/img/avatars/" . $fila["avatar_masterdriver"]) . '.png"
                    class="rounded-circle" style="width: 30px; height: 30px; object-fit: cover;" alt="avatar">
                    </td>
                    <td class="titulostablaverificarcomodatodemo">' . $fila['nombre1masterdriver'] . ' ' . $fila['nombre2masterdriver'] . ' ' . $fila['apellidopaternomasterdriver'] . ' ' . $fila['apellidomaternomasterdriver'] . '</td>
                    <td class="text-center">
